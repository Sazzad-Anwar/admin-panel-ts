...
const User: FC = () => {
    ...
    const { data: user, isLoading } = useSWR<IUser>(`/users/${params?.id}`, fetcher);
            ...
            {isLoading ? (
                ...
                <article className="rounded-md shadow-md p-5 bg-gray-200 dark:bg-primary-dark-600 text-black my-5">
                    ...
                    <div className="flex items-center my-3">
                        ...
                        <h1 className="ml-3 text-2xl dark:text-white">{user && user?.name}</h1>
                    </div>
                    ...
                    <address className="text-xl mb-2 capitalize dark:text-white">
                        Address: {user && user?.address?.street}, {user && user?.address?.city} ,
